...
template< class tm>
void total_wavefunction<tm>::laplacianGradientLogWave(const all_particles_t & p,value_t & e,value_t & eF, grad_t & grad,double & waveValue)
{
  value_t tmp,waveTmp;
  int i,j;
  ...
  for(i=0;i<waves.size();i++)
    {
      waves[i]->laplacianMinusGradientSquaredLogWave(p,grad,tmp,waveTmp);
      e=e+tmp;
      waveValue+=waveTmp;
    }
  //cout << "K:" <<e<<endl;
