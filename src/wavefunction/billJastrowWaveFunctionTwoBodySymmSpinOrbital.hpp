  ...
  virtual void laplacianMinusGradientSquaredLogWave(const all_particles_t & p,grad_t & grad,value_t & e,value_t & waveValue)
{
  ...
  virtual void laplacianMinusGradientSquaredLogWave(const all_particles_t & p,grad_t & grad,value_t & e,value_t & waveValue,vector<double> & spinFlips)
{
  typedef typename grad_t::gradParticles_t gradParticles_t;
  ...
  const particles_t &p1=p[this->src_particle_set];
  gradParticles_t & grad1=grad[this->src_particle_set];
  double x,sign,d,valueSpinFlipi,valueSpinFlipj;
  
  // reset to zero output
  e=0;
  waveValue=0;
  
  for(int i=0;i<spinFlips.size();++i)
    {
      spinFlips[i]=0;
    }
  ...
  for(int i=0;i<p1.size();++i)
      ...
      for(int j=0;j<i;++j)
	{
	  ...
	  valueSpinFlipi=this->jastrowc.d0(d,-p1[i].spin(),p1[j].spin());
	  valueSpinFlipj=this->jastrowc.d0(d,p1[i].spin(),-p1[j].spin());
	  spinFlips[i]+=log(valueSpinFlipi/tmp);
	  spinFlips[j]+=log(valueSpinFlipj/tmp);
